...
if abs(tmp_y - y_start) < 1e-6:
    # Finished
    print("==> Newton Method(BFGS) Algorithm Finished")
...
while tmp_nIteration < 1000:
    ...
    gradient = np.array([nd.Gradient(F)([x1_start, x2_start])]).T
    r = 1 / np.matmul(Y.T, S)
    PART1 = np.matmul(r*S, Y.T)
    PART2 = np.matmul(r*Y, S.T)
    PART3 = np.matmul(r*S, S.T)
    I = np.identity(max(PART1.shape))
    H = np.matmul(np.matmul((I-PART1), H), (I-PART2)) + PART3
    P = -np.matmul(H, gradient) 
...
# Writing results in text file
f = open("results/1C_result" + str([startPoint[0], startPoint[1]]) + ".txt", "w")
f.write(result_string)
f.close()
