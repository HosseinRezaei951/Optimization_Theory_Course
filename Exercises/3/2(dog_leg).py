...
#########################################################################################
# ==> Start trust_region (Dog Leg Method) <==
#########################################################################################
...
def dog_leg(x, b_matrix, delta):
    ...
    p_u = -grad_path_best_alpha * g
    p_b = -np.dot(np.linalg.inv(b_matrix), g)
    tau = delta / np.linalg.norm(p_u, 1)
    if tau <= 1:
        return tau * p_u
    elif tau <= 2:
        return p_u + (tau-1) * (p_b - p_u)
    else:
        p_b_norm = np.linalg.norm(p_b, 1)
        if p_b_norm <= delta:
            return p_b
        else:
...
def trust_region(x, result_string, subproblem_solver):
    ...
    # Start
    print("==> Start trust_region (Dog Leg Method) Algorithm")
    ...
    # Finished
    print("==> Ftrust_region (Dog Leg Method) Algorithm Finished")
    return x, result_string

Results, result_string = trust_region(startPoint, result_string, dog_leg)
print('Results:', Results)

plt.plot(x1_list, x2_list, 'o-r', ms=3)

# Writing results in text file
f = open("results/2(dog_leg)_result" + str(startPoint) + ".txt", "w")
