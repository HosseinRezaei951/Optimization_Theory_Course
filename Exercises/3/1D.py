...
#########################################################################################
# ==> Fletcher-Reeves gradient Method <==
#########################################################################################
...
# Start
print("==> Start Running Fletcher-Reeves gradient Method Algorithm")
tmp_nIteration = 1

# first iteration
gradient_start = np.array([nd.Gradient(F)([x1_start, x2_start])]).T
P = -gradient_start

# print("gradient_start=", gradient_start, gradient_start.shape, "\n")
# print("P=", P, P.shape, "\n")
# input()
...
if abs(tmp_y - y_start) < 1e-6:
    # Finished
    print("==> Fletcher-Reeves gradient Method Algorithm Finished")
...
while tmp_nIteration < 1000:
    ...
    B = np.power(np.linalg.norm(gradient, 2),2) / np.power(np.linalg.norm(gradient_start, 2),2)
    P = -gradient + (B*P) 
...
# Writing results in text file
f = open("results/1D_result" + str([startPoint[0], startPoint[1]]) + ".txt", "w")
f.write(result_string)
f.close()
