...
# Start
print("==> Start Running Newton Method(DFP) Algorithm")
tmp_nIteration = 1
...
if abs(tmp_y - y_start) < 1e-6:
    # Finished
    print("==> Newton Method(DFP) Algorithm Finished")
...
while tmp_nIteration < 1000:
    ...
    gradient = np.array([nd.Gradient(F)([x1_start, x2_start])]).T
    PART1 = np.matmul(np.matmul(H, Y), np.matmul(Y.T, H))
    PART2 = np.matmul(np.matmul(Y.T, H), Y)
    PART3 = np.matmul(S, S.T)
    PART4 = np.matmul(Y.T, S)
    H = H - (PART1/PART2) + (PART3/PART4)
    P = -np.matmul(H, gradient) 
...
# Writing results in text file
f = open("results/1B_result" + str([startPoint[0], startPoint[1]]) + ".txt", "w")
f.write(result_string)
f.close()
